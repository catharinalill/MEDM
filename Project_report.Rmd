...
library(kknn)     #for kknn function
library(GGally)
library(corrplot)
...
#Erase columns which have NA for some observations (WE CAN CONSIDER SETTING THIS CELLS TO ZERO)
...
#Create new column for response variable DELAY: 0 if the flight is not delayed by more than 5 minutes, 1 if the flight is delayed by 5-20 minutes, 2 if the flight is delayed by more than 20 minutes
#Here we consider only the arrival delay  (THIS CHOICE SHOULD BE JUSTIFIED IN THE REPORT!)
temp = ifelse(flights_small$ARRIVAL_DELAY < 6,flights_small$DELAY <- 0,ifelse(flights_small$ARRIVAL_DELAY <21,flights_small$DELAY <- 1,flights_small$DELAY <- 2))
flights_small$DELAY = temp
...
#SHOULD WE CONCIDER THE CATEGORICAL VARIABLES HERE??
```
