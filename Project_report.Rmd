...
#Number of delayed and not delayed flights in original sample
handled_flights_delayed <- handled_flights[handled_flights$ARRIVAL_DELAY>0,]
handled_flights_notdelayed <- handled_flights[handled_flights$ARRIVAL_DELAY<=0,]
...
mostdelayedNB <- colnames(countdelayNB)[which.is.max(countdelayNB[1,])] #Airline which had the higher value of delay
leastdelayedNB <- colnames(countdelayNB)[which.is.max(countdelayNB[2,])] #Airline which had the lower value of delay
...
modelLR <- glm(formula = formula(train_data_y~.), data = train_data, family = binomial, control = list(maxit = 100))

predictLR <- predict(modelLR, test_data)

#Confusion matrix
table(predictLR, test_data_y)
...
### Linear Discriminant Analysis - haven't tried with 2 categories and a smaller dataset, maybe we can try, but Naive Bayes and Decision Tree can run for 100.000 with no problem
