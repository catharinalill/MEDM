...
flights_small <- sample_n(handled_flights, 10000) #(THIS CHOICE SHOULD BE JUSTIFIED IN THE REPORT!)
...
barplot(table(flights_small$DELAY, flights_small$AIRLINE), main="Categories of delay by Airline",
...
##We are supposed to use valid_Data here and not test_Data since w are dividing our data set into 3?? YES
...
modelTREE <- rpart(formula = formula(train_data_y~.), data = train_data, method = "class")
predictTREE <- predict(modelTREE, test_data_x)
