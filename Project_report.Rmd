...
The dataset has 5.819.079 observations of 31 variables. Since this dataset is not prepared for classification, we must do this preparation, taking into account what we want to achieve. The goal is to choose the best airline based on the delays of their flights, ignoring the cancelled and diverted ones. So, we will create a categorical response variable which represents the delay, and choose which variables we want to keep for the classifier.
...
<!--I think it might be unneccecary to list all of these??   

AND we should add `variable name`, because then it is easier to see/read in the text as it gets another font. -->

Since we are ignoring the cancelled and diverted flights we removed the variables: `Diverted`, `Cancelled`, `Cancellation reason`. We noticed that all the flights were from the year 2015. As a result, we eliminate the variable `Year`, as it has no interest to this problem.
...
When taking a flight, the most common concern is to arrive on time, independently of the delay that may have occurred in the departure. So, we only take into consideration the `Arrival delay` to build the response variable. 

Since we want to build a classifier, we need to define the categories for the response variable. To do this, we analyzed the delayed flights mean and median. 
(HISTOGRAM, BOXPLOT)
The results were $mean = 33.56$ and $median = 15$. From this, we defined the following categories of `DELAY`:
...
<!-- Becomes a point-list when knitting -->
...
flights_small <- sample_n(handled_flights, 50000) #(THIS CHOICE SHOULD BE JUSTIFIED IN THE REPORT!)
...
#Define only 2 categories (None, Delayed) instead of 4
...
counts <- table(flights_small$DELAY, flights_small$AIRLINE)
barplot(counts, main="Categories of delay by Airline",
...
#IF we end up only using 2 categories for the response variable we can only have 2 colours here!
...
After exploring and processing the data according to our goal, the classification problem will have 11 features, where 8 are categorical, these are `Month`, `Day`, `Day of week`, `Airline`, `Flight number`, `Tail number`, `Origin airport`, `Destination airport`, and 3 are continuous, which are `Schedule departure`, `Schedule time` and `Scheduled arrival`.
...
# NOW THIS IS MAYBE NOT SO IMPORTANT SINCE ALMOST all our variables are categorical
...
fitKNN <- fitted(pred.knn.m1)

predictKNN <- predict(fitKNN, test = test_data_x)
...
#Fit model - I guess this isn´t correct because is designed for a binary problem!

# model_logreg <- glm(DELAY~. ,data = train_data, family = binomial, control = list(maxit = 100))
# summary(model_logreg)
# tidy(model_logreg, exponentiate = TRUE, conf.level = 0.95)
# 
# #From this we choose the significant variables using a significance level of 0.05
# 
# probabilities <- model_logreg %>% predict(test_data, type = "response")
# 
# predicted_classes <- ifelse(probabilities > 0.5, "1", "0")
# 
# 
# confusion_matrix <- table(Predicted = predicted_classes, Reference = test_data$DELAY)
# confusion_matrix
# 
# #Calculate Accuracy
# mean(predicted_classes == test_data$DELAY)
...
```{r , echo=TRUE}
#Variable selection to reduce computation time and not run out of memory


#Fit model
modelLDA <-lda(formula = formula(train_data_y~.), data = train_data)
...
modelTREE <- rpart(formula = formula(train_data_y~.), data = train_data_x, method = "class")
#Works with at least 50 000 obs if we use 2 categories for the response variable
summary(modelTREE)
