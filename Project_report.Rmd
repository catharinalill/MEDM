...
# ROC-CURVES
roc(test_data_y, as.numeric(predictNB), legacy.axes = TRUE, auc = TRUE, plot = TRUE)
#Think the curve looks a bit wierd, and the axis for the specificity is from -0.5 to 1.5 which is wierd
...
```{r , echo=TRUE}
# MODEL FITTING
modelTREE <- rpart(formula = formula(train_data_y~.), data = train_data_x, method = "class", parms = list(split ="gini")) #instead of information, we may use "gini"
...
predictTREE <- predict(modelTREE, test_data, type = "class")
...
printcp(modelTREE)
bestcp <- modelTREE$cptable[which.min(modelTREE$cptable[,"xerror"]), "CP"]
plotcp(modelTREE)
...
#THE AUC = 0.5 as well, so not better as by chance which is not good...
...
modelRF = randomForest(train_data_y~., data = train_data_x_rf, mtry = 3, importance = TRUE)
