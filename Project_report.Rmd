...
flights_small <- sample_n(handled_flights, 1000) #(THIS CHOICE SHOULD BE JUSTIFIED IN THE REPORT!)
...
##We are supposed to use valid_Data here and not test_Data since w are dividing our data set into 3??

#Make model
ks = 1:10 #Choose K from 1 to 10
classification_errors = sapply(ks,function(k) {
  pred.knn <- knngow(formula = formula(train_data[,12]~.), train = train_data, test = valid_data, k = k, distance = 1)
  fit <- fitted(pred.knn)
  mean(valid_data[,12] != fit)   #I think this is the accuracy, but it should be the classification variables
})

####ANother attempt at calculating class_error
...
#WE need to use the library dprep for the knngow function, but we are not able to download this... 
...
pred.knn.m1 <- knngow(formula = formula(train_data[,12]~.), train = train_data, test = test_data, k = k_optimal, distance = 1)
...
#Fit model
model_lda <- multinom(DELAY~. ,data = train_data)
summary(model_lda)

probabilities <- model_lda %>% predict(test_data, type = "response")
...
library(rpart)
tree <- rpart(DELAY ~., data = train_data, method = "class")
