...
#The first variable in our flights dataset is YEAR = 2015, which is equal for all so we should not include this in the data analysis/classification.
...
flights_small <- sample_n(handled_flights, 1000)      #1000000                                                    #(THIS CHOICE SHOULD BE JUSTIFIED IN THE REPORT!)
...
# (THIS CHOICE SHOULD BE JUSTIFIED IN THE REPORT!)
...
#So, we must give to the classifier, only the variables corresponding to subjects known a priori: scheduled departure, distance, etc.
...
flights_to_predict$DISTANCE <- NULL    ## THIS YOU SAY WE SHOULD KEEP IN THE TEXT ABOVE???
...
#train_data$DELAY <- NULL     #WHY DO WE NEED TO DO THIS???
...
#test_data$DELAY <- NULL    #WHY DO WE NEED TO DO THIS???
...
classification_errors = sapply(ks,function(k) {
  ...
  fit <- fitted(pred.knn)
  mean(test_data[,12] == fit)   #I think this is the accuracy, but it should be the classification variables
})
classification_errors

plot(ks, classification_errors, type="o", pch = 16, ylim = c(0, 0.7), col = co[2],
     xlab = "Number of neighbors", ylab="Misclassification error")

#MAYBE K = 15 is a good choiche from looking at the plot?
