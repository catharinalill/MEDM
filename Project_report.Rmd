...
#Create nem column (DELAY: 0 if the flight is not delayed, 1 if the flight is delayed) 
##Here we consider only the arrival delay
handled_flights$DELAY <- handled_flights$ARRIVAL_DELAY

handled_flights$DELAY <- ifelse(handled_flights$DELAY > 0,handled_flights$DELAY <- 1,handled_flights$DELAY <- 0)
...
indxTrain <- createDataPartition(y = flights[,33],p = 0.8,list = FALSE)
train.data <- genes[indxTrain,]    ### WHY genes?????
valid.data <- genes[-indxTrain,]   ### ISN'T IT test.data????
...
pred.knn.cv <- train(CL~.,
                     ...
                     metric     = "Accuracy",
                     data       = cbind(CL = as.factor(train.data[,33]), train.data[,-33]))
...
fit_cv <- predict(pred.knn.cv,newdata = test.data)  ### WHERE IS test.data defined?
...
pred.knn.cv <- train(CL~.,
                     ...
                     trControl  = train.control,
                     metric     = "Accuracy",
                     data       = cbind(CL = as.factor(train.data[,23]), train.data[,-2]))
